...
- **Git 集成** — 查看提交历史、分支信息、仓库状态；分支管理（创建/删除/重命名本地分支，`-d` 失败二次确认走 `-D` 强删，当前分支禁删）；双击变动文件弹窗双栏对照查看 diff、选择性提交（commit）、推送到远程（push）；单文件暂存/取消暂存（行级 +/- 按钮 + 批量「更多」下拉，`git restore --staged` 统一命令通配已跟踪/未跟踪）；标签管理（列出/创建轻量+注释标签/删除/推送单个与全部）；远程仓库管理（增删 remote、fetch 可选 --prune、为当前分支设跟踪上游）；选中 git 工作目录即可在右栏直接查看仓库详情，左栏工作目录列表标记 git 仓库
