...
- **文件操作** — 新建、重命名、删除、预览，支持在资源管理器/VSCode/Warp/Obsidian 中打开；「拷贝到」支持自定义目标文件名（默认与原名一致，同名冲突自动加 (1)），拷贝自动去除目标只读属性
...
- **Git 集成** — 查看提交历史（支持按作者/日期区间/文件路径/关键词服务端过滤，覆盖超 500 条提交；纯内存缓存 + HEAD SHA 增量更新避免重复扫描与 getCommitFiles 重算）、分支信息、仓库状态；分支管理（创建/删除/重命名本地分支，`-d` 失败二次确认走 `-D` 强删，当前分支禁删）；双击变动文件弹窗双栏对照查看 diff、选择性提交（commit）、推送到远程（push）；单文件暂存/取消暂存（行级 +/- 按钮 + 批量「更多」下拉，`git restore --staged` 统一命令通配已跟踪/未跟踪）；标签管理（列出/创建轻量+注释标签/删除/推送单个与全部）；远程仓库管理（增删 remote、fetch 可选 --prune、为当前分支设跟踪上游）；合并 / 变基 / 拣选 / 冲突解决（合并支持 ff/no-ff/squash 三策略，合并前校验工作区干净；变基支持 `--continue`/`--abort`/`--skip`；cherry-pick 单提交拣选；pull 入口可选 `--rebase` 模式；冲突文件列表 + 标记已解决 `git add` + continue/abort，不做内置三方合并编辑器）；提交历史 diff（展开提交详情点击变更文件查看该提交相对父提交的双栏 diff，root commit 对比空树呈现全增；列表勾选两个提交生成区间 diff，按文件分组双栏展示，二进制文件降级提示）；选中 git 工作目录即可在右栏直接查看仓库详情，左栏工作目录列表标记 git 仓库；变更类 Git 操作按仓库路径串行化（同仓操作进行中时拒绝并 warning 提示稍后重试，跨仓并行，只读不受影响）；子模块管理（查看列表/四色状态标签/detached 警告/初始化/更新 checkout-merge-rebase-remote 四模式/添加/删除三步清理/detached 切换跟踪分支）
- **仓库统计** — 活动栏一级入口，复用提交历史缓存聚合可视化（不重复扫 git log）：提交趋势图（按日/周/月聚合，固定档位 7天/30天/90天/1年/全部，粒度随档位自动适配）、活跃度热力图（类 GitHub 草地，最近一年按日，5 档色阶）、贡献者提交数排名（按提交数降序）；超限仓库（>5000 commit）采样最近 5000 条并提示；基于 ECharts 按需引入渲染；统计页顶部展示当前仓库信息条（仓库名/路径/分支/远程地址，复用仓库信息缓存）；工作目录与文件树右键「跳转仓库统计」一键跳转（仅 git 仓库节点）
- **批量更新** — 一键批量 pull 所有仓库，自动跳过未配置远程的本地仓库
- **全局状态看板** — 活动栏一级入口，手动 pin 关注的核心仓库（不全局扫描，开源仓等不关注的不进看板），一眼看清每个仓库的工作区（干净/有改动）、未推送提交数（ahead）、未拉取提交数（behind）、分支与上游状态；ahead/behind 基于本地远程引用（上次 fetch/clone 快照）计算，看板不主动 fetch，behind 为相对上次 fetch 的近似值并标注时效；点击行跳转该仓（切工作目录 + 展开文件树定位）；双入口添加 pin（看板「添加仓库」弹窗跨工作目录批量勾选 + 工作目录/文件树右键「加入状态看板」单仓随手 pin），pin 列表持久化 `data/dashboard_pinned.json`
- **仓库筛选器** - 文件树工具栏按钮 / 空白区域右键菜单打开弹窗，扫描当前工作目录下所有 Git 仓库：支持工作目录切换、按仓库名/路径搜索、标签 OR 筛选、已编辑/未编辑双 Tab 分类（有标签即已编辑）、左栏虚拟滚动列表（@vueuse/core useVirtualList）、右栏编辑自定义简述（防抖 800ms 自动保存）与标签（增删即时保存）、README 摘要展示、失效仓库灰显与一键清理、点击跳转自动展开文件树定位目标节点（跨工作目录自动切换再定位）
...
│           ├── LocalChanges.vue         # 本地变更列表（单表分组 + 行级暂存）
│           ├── GitBranches.vue          # Git 分支管理（创建/删除/重命名）
│           ├── GitTags.vue              # Git 标签管理
│           ├── GitRemotes.vue            # Git 远程仓库管理
│           ├── GitMerge.vue             # Git 合并/变基/拣选/冲突解决
│           ├── GitSubmodules.vue       # Git 子模块管理（查看/状态/初始化/更新/添加/删除/detached 切换）
...
| 文档 | 说明 |
| ---- | ---- |
| [快速入门](docs/快速入门.md) | 新用户上手指南：安装、配置、核心流程 |
| [功能说明](docs/功能说明.md) | 功能详情 |
| [架构设计](docs/架构设计.md) | 分层架构、Wails 桥接模型、跨层契约、错误处理与测试架构 |
| [API参考](docs/API参考.md) | Wails 绑定方法清单（139 个方法按域分组）与数据模型 |
| [开发工作流](docs/开发工作流.md) | 开发、测试、构建流程 |
| [测试策略](docs/测试策略.md) | 测试规范 |
| [部署说明](docs/部署说明.md) | 生产构建与分发 |
| [开发规范](docs/开发规范.md) | 代码风格与提交规范 |
| [贡献指南](docs/贡献指南.md) | 环境准备、分支与提交、PR 流程、规范沉淀 |
| [常见问题](docs/常见问题.md) | 开发与使用问题排查 |
| [路线图](docs/路线图.md) | 发展规划 |
| [规范沉淀](docs/spec/README.md) | 跨层契约等项目规范索引 |
