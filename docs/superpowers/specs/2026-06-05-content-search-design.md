...
| 输入 | 范围 | 触发方式 |
|---|---|---|
| `:keyword` | 当前选中的工作目录 | 即时搜索（300ms 防抖） |
| `:path/ keyword` | 当前工作目录下指定子目录 | 即时搜索（300ms 防抖） |
| `::keyword` | 所有工作目录 | Enter 确认后搜索 |
...
1. 显示提示：「将在 N 个工作目录中搜索 "keyword"，预计耗时较长」
2. 用户按 **Enter** 或点击「确认搜索」按钮后开始搜索
3. 搜索过程中显示实时进度：「正在搜索 channel-ab-service (3/15)...」
...
1. **全局搜索确认提示**（`mode === 'content-global'` 且未开始搜索时）
