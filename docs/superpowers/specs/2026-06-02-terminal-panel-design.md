...
为 Git Manager 添加内置终端面板功能，采用 **xterm.js + Go PTY** 方案，在应用底部嵌入一个完整的终端实例。终端工作目录自动跟随文件树选中项，支持多种 Shell 类型配置，面板可拖拽调节高度、可收起/展开。此功能将 Git Manager 从单一的 Git 仓库管理工具向开发者效率平台方向扩展。
...
本设计为 Git Manager 新增内置终端面板功能，采用 **xterm.js + Go PTY** 技术方案，核心特性包括：
