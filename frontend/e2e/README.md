...
- **`window.runtime`**：`wailsjs/runtime/runtime.js` 的 EventsOn / BrowserOpenURL
  等转发到 `window.runtime`，注入 stub（常驻组件 setup 同步调用，缺失即渲染中断）。
  EventsOn 系列将回调登记到页面全局 `window.__wailsEventHandlers`，供
  `{__events__}` 描述符派发事件（模拟 AI 任务 queued/started/output/done 异步事件流）。
...
2. 需要特定返回值时优先在 `wails-mock-defaults.js` 补充；用例级动态行为（序列 / 错误）
   ...
   `{__value__, __events__}`（resolve 值 + 按序派发 Wails 事件，`delayMs` 可编排延迟，
   每次调用完整派发）描述符——每测试独享新页面，覆盖互不影响
